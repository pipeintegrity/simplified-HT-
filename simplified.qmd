---
title: "A Simplified Implementation to Estimate the Upper Shelf Energy and Transition Temperature from Limited Charpy V-Notch Data Sets"
author: "Switzner, Anderson, Martin, Rosenfeld, Ali, Patrick, Veloo"
format: docx
editor: visual3
...
Since the ability to solve this non-linear regression is relying on numerical optimization methods, it requires initial starting values for the four coefficients. The down-side to these methods is that if the initial values are poor or if there are several outliers or if the data is incomplete, the solution can fail to converge or get "stuck" in a local minimum and return a solution that is far from optimal. Although most algorithms have methodologies to avoid getting stuck in a local minimum it is common for the NLS to fail with poor initial values especially when there are outliers in the data as well.

However, with some of simplifying assumptions, the non-linear hyperbolic tangent can be transformed to a linear regression problem that does have a closed-form solution to minimize the RSS through ordinary least squares regression (OLS) of the form $y_i = \alpha x_i + \beta + \epsilon_i$ where $\epsilon_i$ is the residual error. The benefit to the OLS regression methodology is that since the solution is closed-form it is guaranteed to find a solution for the data, in addition it can be done in a spreadsheet or even by hand if needed. In the OLS solution, $\alpha$ and $\beta$ are estimated such as to minimize $\epsilon_i$. 

Since it is a given that the minimum and maximum SA is physically bounded by 0 and 100 percent therefore $A + B\leq 100$ and $A - B \geq 0$. Also looking at @fig-HT it can be seen that the value of $A$ and $B$ are similar with $A$ being only slightly larger than $B$. Therefore a reasonable initial estimate for $A$ and $B$ for the SA solution would be about 51 and 49 respectively. Now that $A$ and $B$ are fixed values, that leaves the @eq-HT with only two unknowns and rearranging the terms and solving for temperature produces the more useful form of $T = C \ atanh\left(\frac{SA - A}{B}\right)+ D$. Since everything in the argument for the $atanh$ is known, the entire $atanh$ term becomes an independent variable $k_1 = atanh\left(\frac{SA -A}{B}\right)$ and the HT function is transformed into a linear equation, $T = C\ k_1+D$ which can be solved by OLS regression. The solution to this will produce two coefficients, slope and intercept corresponding to the $C$ and $D$ respectively. Referring to @fig-HT observe that $D$ is the center of the linear transition area between the upper and lower asymptote and and appears to be approximately the same for SA or AE when looking at @fig-pltdata. Using this assumption and the solution from SA, @eq-HT can again be linearized to be solved by OLS regression. Since $C_{sa}$ and $D_{sa}$ (subscript indicates that they are from the SA solution) are known from the SA solution, inserting them into @eq-HT, all the terms in the $tanh$ function are known and it can be reduced to $k_2 = tanh\left(\frac{T-D_{sa}}{C_{sa}}\right)$ and the HT equation becomes, $AE = A + B \ k_2$ which can also be solved with OLS regression where the slope and intercept from the solution is equal to $B$ and $A$ respectively.

Using the same simplifying assumptions mentioned in this section, the problem can also be solved using NLS for the two remaining coefficients. This can be helpful if the NLS solution does not converge when trying to solve all four coefficients at once for some reason. Though the linearized form of the HT is functionally equivalent to the two term hyperbolic tangent the OLS solution can be influenced by data points that are far from the mean and have high residuals, what are known as high *leverage* points. These single points can have disproportionate influence on the slope of the OLS regression as where they have far less influence in a NLS regression solution since the high leverage points will be in the region that is asymptotic to the minimum or maximum values. In the cases where high leverage points are present, either removing the high residual observation(s) if they seem to be erroneous or using the two term NLS solution may be preferable. In @sec-lodata the solutions for absorbed energy and shear area are shown with a set of data that has significant scatter.

# Results

While this appears to be a workable solution, it does involve some assumptions. To judge the suitability of the simplified solutions it will be used to solve the HT for the example data shown in @fig-pltdata and the results will be compared to the general solution where all four coefficients are solved simultaneously. Since any model is an approximation to a physical phenomena and any measurement is going to have some amount of error regardless of precision, there is uncertainty in the solutions which are represented by the standard errors for the coefficients. Note that since $A$ and $B$ were fixed at a value for the SA solution, there is no confidence intervals or standard error for the OLS solution for these so the standard error and confidence intervals are set to zero for $A$ and $B$ for the SA solution. The results are plotted in @fig-sa.coefsCI and a listing of the coefficients and confidence intervals for the SA solutions is shown in @tbl-results. Viewing the confidence intervals for $A$ and $B$ for the general solution, it appears that 51 and 49 were reasonable assumptions for $A$ and $B$ since they are within the confidence intervals for both coefficients.
...
This is a plot of the confidence intervals for $C$ and $D$ for all three solutions with the estimated value plotted a single point for each. Recall that for the simplified solutions $A$ and $B$ were fixed values and do not have a confidence interval.

```{r}
#| label: fig-sa.coefsCI
#| fig-cap: "Confidence Intervals for Shear Area Solutions"
#| fig-width: 6.5
...
Using the values from all three solutions they were used to plot the HT for each one. Though there are visible differences in the model fit that can be seen in @fig-solplt especially through the transition area. However, at the 85% shear appearance area the two models start to converge though some differences are still evident. To demonstrate these differences, the 95% confidence intervals are shown in @fig-satterr.
...
This plot compares the confidence intervals of the SATT for the two methods. Since the four coefficients are known and the SATT is by definition the temperature at 85% SA, the SATT can be calculated bt solving the HT for temperature and putting 85 in for the SA.
$$
SATT = atanh\left(\frac{85 -A}{B}\right)C+ D
$$ {#eq-satt}

The mean estimated SATT is nearly equal for the three methods but the width of the confidence interval for the simplified methods is smaller due to $A$ and $B$ being fixed with no error associated with them. Since the two 95% confidence intervals overlap it indicates that given the uncertainty in either model, the differences in the predicted SATT are not significant.
...
The next comparison is with the simplified methods and the general solution for AE. @tbl-aecoefs is the coefficients for all three models. Note that the $C$ and $D$ coefficients are carried over from the SA solution for the simplified methods.
...
This plot will show the HT solutions for the three solutions for comparison. With consistent data, the three solution methods are all approximately the same. See @fig-ae-error for a comparison of the uncertainty in the upper shelf estimates.
...
cvn_data %>%
  ...
  geom_hline(
    yintercept = tdy_cvn$estimate[1] + tdy_cvn$estimate[2],
    lty = 2,
    lwd = 0.75,
    col = 'grey50'
  ) +
  ...
  annotate(
    "text",
    x = 0,
    y = tdy_cvn$estimate[1] + tdy_cvn$estimate[2],
    label = "Upper Shelf",
    vjust = 1
  ) +
  labs(x = "Temperature (\u00B0F)",
...
# Less Optimal data {#sec-lodata}

In this section, a set of data that exhibits significant scatter and inconsistencies is shown against the three types of solutions for both shear area and absorbed energy. In @fig-badsa-data it can be seen that there are several challenges in this data set. The SA decreases between 140 and 145°F and the percent SA jumps from about 50% to 100% over a change of 5°F. For the simplified solutions the SATT is approximately within$\pm$ 10°F of the general solution. Which is probably not a difference of any practical considerations.
...
Even though the solutions produced by simplified methods were not very close to the general solution, in both cases they produced conservative answers. By removing the inconsistent data at 145°F and refitting the models, it brought the upper shelf AE for the simplified OLS model about 20% closer to the general solution. The simplified HT and general model were largely unaffected by removing the outliers.
...
# Conclusion 
This paper has shown a method of fitting CVN data to the hyperbolic tangent model that is conservative as well as repeatable and allows for quantification of the uncertainty so the user can make informed decisions about the operation of a pipeline.
